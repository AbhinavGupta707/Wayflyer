...
def siblings_in_stock(variant_id: str, at: TimePoint = None) -> list[dict]:
    """Other-size variants of the SAME product and colourway, in stock (>0) as of `at`.

    Returns Contract-A `Sibling` dicts {variant_id, size, stock, price}, ordered along
    the product's size ladder, excluding the queried variant itself. Scoped to the same
    colour (products carry size x colour variants), so an exchange offer never silently
    swaps the customer's colourway. Returns [] if the variant is unknown.
    """
    ...
    for sib_id in _variants_by_product().get(row["product_id"], []):
        ...
        if sib.get("colour", "") != row.get("colour", ""):
            continue
